...
void empty_btn_cb(lv_event_t * t){
  printf("I'm an empty callback!\n");
}
void another_cb(lv_event_t * t){
  printf("I'm Another CB!\n");
}
...
counter_user_data temp_data;
counter_user_data time_data;
...
void start_alpha_animation(lv_anim_enable_t * status, lv_obj_t * obj, int32_t duration, int32_t delay){
  ...
  lv_anim_set_exec_cb(&a, anim_alpha_cb);
  lv_anim_start(&a);
  *status = LV_ANIM_ON;
}

void stop_alpha_animation(lv_anim_enable_t * status, lv_obj_t * obj){
  if (*status == LV_ANIM_OFF){
    return;
  }

  printf("Stopping animation\n");
  lv_anim_del(obj, anim_alpha_cb);
...
void main_screen_init(){
  time_data = counter_data_create(oven_program->setDurationMinutes);
  temp_data = counter_data_create(oven_program->setTemperature);
  ...
  lv_label_set_text_fmt(program_name, "%s", oven_program->name);
  ...
  time_update_lb(oven_program->setDurationMinutes, time_h_lb, time_m_lb);
  ...
  lv_label_set_text_fmt(temp_lb, "%d", oven_program->setTemperature);
...
void temp_msgbox_open(lv_event_t *e){
  ...
  lv_label_set_text_fmt(temp_msgbox_lb, "%d", oven_program->setTemperature);
...
void temp_msgbox_btn_cb(lv_event_t *e){
  oven_program->setTemperature = *temp_data.value;
  lv_label_set_text_fmt(temp_lb, "%d", oven_program->setTemperature);
...
void time_msgbox_btn_cb(lv_event_t *e){
  oven_program->setDurationMinutes = *time_data.value;
  time_update_lb(oven_program->setDurationMinutes, time_h_lb, time_m_lb);
  lv_msgbox_close(lv_event_get_current_target(e));
}
...
void main_screen_update(lv_timer_t * timer){
  ...
  if (oven_program->started){
    start_alpha_animation(&time_separator_anim_status, time_separator_lb, 1000, 200);
    time_update_lb(oven_program->remainingMinutes, time_h_lb, time_m_lb);
  }
  else {
    stop_alpha_animation(&time_separator_anim_status, time_separator_lb);
    time_update_lb(oven_program->setDurationMinutes, time_h_lb, time_m_lb);
