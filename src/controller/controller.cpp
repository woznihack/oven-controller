...
oven_light_state_node_t light_sm[] = { // light state machine
  {OVEN_S_LIGHT_ON, light_on},
  {OVEN_S_LIGHT_OFF, light_off},
};

oven_fan_state_node_t fan_sm[] = { // fan state machine
  {OVEN_S_FAN_ON, fan_on},
  {OVEN_S_FAN_OFF, fan_off},
};

oven_top_heater_state_node_t top_heater_sm[] = { // light state machine
  {OVEN_S_TOP_HEATER_ON, top_heater_on},
  {OVEN_S_TOP_HEATER_OFF, top_heater_off},
};

oven_bottom_heater_state_node_t bottom_heater_sm[] = { // light state machine
...
uint16_t cooking_duration_m = 9;
uint16_t cooking_remaining_m;
...
void oven_idle(){
  ...
  q_enqueue(ui_queue, UI_OVEN_STARTED, NULL);
}
...
void oven_preheat(){
  ...
  // EMULATE PREHEAT COMPLETE JUST IF 30 seconds passed from beginning
  bool preheat_complete = (difftime(time(NULL), preheat_started_at) >= 10);
  if (preheat_complete) {
...
void oven_mantain(){
  ...
  int remaining_m = cooking_duration_m - (int)floor(difftime(time(NULL), cooking_started_at)/10);
  if (remaining_m == 0) {
  ...
  q_enqueue(ui_queue, UI_UPDATE_REMAINING_MINUTES, &remaining_m);
}
